...
void round_robin(struct Process p[], int n, int time_quantum) {
    ...
    while (completed != n) {
        int any_process_executed = 0; // Track if any process executed in this iteration

        for (int i = 0; i < n; i++) {
            if (p[i].arrival_time <= current_time && p[i].remaining_time > 0) {
                any_process_executed = 1; // A process was found to execute
        ...
        if (!any_process_executed) {
            current_time++; // If no process executed, increment time to handle idle time
        }
    }
